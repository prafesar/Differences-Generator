...
const renderNodeAction = [
  {
    type: 'removed',
    render: (path) =>
      `Property '${pathToStr(path)}' was removed`
  },
  {
    type: 'added',
    render: (path, { valueAfter }) =>
      `Property '${pathToStr(path)}' was added whith value: ${stringify(valueAfter)}`
  },
  {
    type: 'updated',
    render: (path, { valueBefore, valueAfter }) =>
      `Property '${pathToStr(path)}' was updated. From ${stringify(valueBefore)} to ${stringify(valueAfter)}`
  },
];
